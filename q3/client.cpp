...
void *user_routine(void *arg)
{
    ...
    int num_bytes_read;
    tie(output, num_bytes_read) = read_string_from_socket(socket_fd, buff_sz);
...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < num_requests; i++)
    {
        ...
        cin >> time;
        getline(cin, request);
        request = to_string(time) + request;
        // if (request.back() == '\n')
        if (i != num_requests - 1)
            request.pop_back();
        requests.pb(request);
    ...
    for (int i = 0; i < num_requests; i++)
    {
        ...
        *index = i;
        pthread_create(&users[i], NULL, user_routine, index);
    }
    for (int i = 0; i < num_requests; i++)
    {
